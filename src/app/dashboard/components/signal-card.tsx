...
function buildContext(signal: Signal): string {
  const lines: string[] = [
  ...
  if (signal.themes && signal.themes.length > 0) {
    lines.push(`**Themes:** ${signal.themes.join(', ')}`)
  }
  lines.push('', '## Reasoning', signal.reasoning)
  if (signal.suggestedAction) {
    lines.push('', '## Suggested Action', signal.suggestedAction)
  }
  if (signal.notes) {
    lines.push('', '## Notes', signal.notes)
  }
  lines.push('', `*${signal.evidence.length} supporting input${signal.evidence.length !== 1 ? 's' : ''} — open signal detail for full evidence.*`)
  return lines.join('\n')
}
